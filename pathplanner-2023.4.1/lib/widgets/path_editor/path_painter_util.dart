...
class PathPainterUtil {
  ...
  static void paintRobotOutline(
    ...
    double notchWidth = metersToPixels(0.177, scale, fieldImage);
    double notchLength = metersToPixels(0.127, scale, fieldImage);
    double armWidth = metersToPixels(0.0381, scale, fieldImage);
    double armLength = metersToPixels(1.2446, scale, fieldImage);
    ...
    Offset notchLeft = Offset(center.dx + (notchWidth * sin(angle)),
        center.dy - (notchWidth * cos(angle)));
    Offset notchRight = Offset(center.dx - (notchWidth * sin(angle)),
        center.dy + (notchWidth * cos(angle)));

    Offset notchFrontLeft = Offset(
        notchLeft.dx + (halfLength * cos(angle)), notchLeft.dy + (halfLength * sin(angle)));
    Offset notchFrontRight = Offset(
        notchRight.dx + (halfLength * cos(angle)), notchRight.dy + (halfLength * sin(angle)));
    Offset notchBottomLeft = Offset(
        notchLeft.dx + (notchLength * cos(angle)), notchLeft.dy + (notchLength * sin(angle)));
    Offset notchBottomRight = Offset(
        notchRight.dx + (notchLength * cos(angle)), notchRight.dy + (notchLength * sin(angle)));
    ...
    canvas.drawLine(notchFrontLeft, notchBottomLeft, paint);
    canvas.drawLine(notchBottomLeft, notchBottomRight, paint);
    canvas.drawLine(notchBottomRight, notchFrontRight, paint);

    Offset theWrist = Offset(center.dx - (armWidth * sin(angle)) + (armLength * cos(angle)),
        center.dy + (armWidth * cos(angle)) + (armLength * sin(angle)));

    canvas.drawCircle(theWrist, PathPainterUtil.uiPointSizeToPixels(15, scale, fieldImage), paint);
    paint.color = Colors.purple;
    canvas.drawCircle(theWrist, PathPainterUtil.uiPointSizeToPixels(2, scale, fieldImage), paint);
